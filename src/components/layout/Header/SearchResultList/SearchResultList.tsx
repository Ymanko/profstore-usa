...
export const SearchResultList: FC<TSearchResultListProps> = ({ searchData, isFocus }) => {
  ...
  if ((!safeSearchData.products.length && !safeSearchData.collections.length) || !isFocus) {
    return null;
  }
      ...
      {safeSearchData.products.length !== 0 && (
        <>
          ...
          <ul className={s.searchResultList}>
            {safeSearchData.products.map(({ id, title, priceRange, images, handle }) => {
                ...
                <li key={id} className={s.searchResultListItem}>
                  <Link href={`/catalog/${handle}`} className={s.searchResultListLink}>
                    ...
                    <div>
                      ...
                      <p className={s.searchResultListItemPrice}>{priceRange.maxVariantPrice.amount}$</p>
                    </div>
                  </Link>
                </li>
              );
